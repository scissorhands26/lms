...
export async function updateAnswerInDB(
  ...
  const doesAnswerExist = await pb.collection("exam_answers").getFullList({
    filter: `question = "${question.id}" && attempt = "${attempt.id}"`,
  });

  if (doesAnswerExist.length > 0) {
    await pb.collection("exam_answers").update(doesAnswerExist[0].id, {
      answer,
    });
  } else {
    await pb.collection("exam_answers").create({
      user: pb.authStore.model?.id,
      exam: attempt.expand.exam.id,
...
export async function navigateToExam(exam: any) {}
